...
class MeliConfig(models.Model):
    ...
    @api.model
    def _cron_poll_claims(self):
        configs = self.search([
            ('active', '=', True), ('state', '=', 'connected'),
        ])
        for config in configs:
            try:
                config._poll_recent_claims()
            except Exception:
                _logger.exception(
                    "Mercado Libre claim polling failed for config %s",
                    config.id,
                )
    ...
    _CLAIMS_SEARCH_PAGE_LIMIT = 100
    # Safety cap against a runaway date range (the manual recovery
    # wizard lets a person type an arbitrary range) — 50 pages of 100 is
    # 5,000 claims, well under Mercado Libre's own offset+limit < 10000
    # ceiling for this endpoint.
    _CLAIMS_SEARCH_MAX_PAGES = 50

    @staticmethod
    def _meli_claims_search_timestamp(dt):
        # /post-purchase/v1/claims/search requires millisecond precision
        # on range boundaries (confirmed in the official docs) — unlike
        # /orders/search, which only supports hour granularity. The
        # offset must be colon-less (+0000, ISO 8601 "basic" form): a
        # real call using "+00:00" (the "extended" form, which the
        # docs' own prose oddly also shows) got HTTP 400 from the real
        # API (2026-09-04) — Mercado Libre's own worked example for
        # this exact endpoint uses "-0300" (no colon), which is what
        # this now matches.
        return dt.strftime('%Y-%m-%dT%H:%M:%S.000+0000')

    def _meli_search_claim_ids(self, date_from, date_to):
        """Paginates /post-purchase/v1/claims/search for this
        connection's seller (players.role=respondent) across
        [date_from, date_to) on last_updated, returning every claim id
        found. Shared by _poll_recent_claims (a small rolling window,
        normally a single page) and meli.claim.recovery.wizard (an
        arbitrary, possibly wide, range someone picks by hand to
        recover from a past outage).

        site_id is required here even though it isn't conceptually a
        filter for THIS query: confirmed against the real API
        (2026-09-04) that `range` does NOT count as a filter on its own
        for the "atLeastOneFilterProvided" validation, and neither does
        players.user_id + players.role together — despite the official
        docs' own text implying otherwise. site_id is the cheapest
        param that satisfies that check without narrowing the actual
        result set (unlike, say, status=opened, which would silently
        drop newly-closed claims from the window).
        """
        self.ensure_one()
        claim_range = (
            f"last_updated:after:{self._meli_claims_search_timestamp(date_from)},"
            f"before:{self._meli_claims_search_timestamp(date_to)}"
        )
        claim_ids = []
        offset = 0
        for _page in range(self._CLAIMS_SEARCH_MAX_PAGES):
            data = self._api_get('/post-purchase/v1/claims/search', params={
                'players.user_id': self.ml_user_id,
                'players.role': 'respondent',
                'site_id': self.site_id,
                'range': claim_range,
                'limit': self._CLAIMS_SEARCH_PAGE_LIMIT,
                'offset': offset,
            }) or {}
            ...
            offset += self._CLAIMS_SEARCH_PAGE_LIMIT
            total = (data.get('paging') or {}).get('total', 0)
            if not results or offset >= total:
                break
        return claim_ids

    def _poll_recent_claims(self):
        """Safety-net polling for reclamos/devoluciones — the same idea
        as _poll_recent_orders, applied to post_purchase notifications:
        catches a claims/claims_actions webhook that was never
        delivered or failed. Checkpoint-based (last_claim_poll_at),
        with the same overlap margin, so an outage of any length never
        leaves a permanent gap. Every claim id found is reimported
        through the same identity_key the webhook itself uses, so a
        claim already being processed from a live notification is never
        double-queued.
        """
        self.ensure_one()
        now = fields.Datetime.now()
        since = self.last_claim_poll_at or fields.Datetime.subtract(
            now, hours=self._POLL_DEFAULT_LOOKBACK_HOURS,
        )
        since = fields.Datetime.subtract(since, minutes=self._POLL_OVERLAP_MINUTES)
        claim_ids = self._meli_search_claim_ids(since, now)
