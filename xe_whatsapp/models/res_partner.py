...
class ResPartner(models.Model):
    ...
    def action_open_partner_wa_channels(self):
        return {
            ...
            'views': [(self.env.ref('xe_whatsapp.discuss_channel_view_list_whatsapp').id, 'list')],
        }

    def update_whatsapp_partner(self, user_id):
        team_member = self.env['crm.team.member'].get_team_member(user_id)
        if team_member:
            query = """
                UPDATE res_partner
                SET user_id = %s, company_id = %s, team_id = %s
                WHERE id = %s
            """
            params = (user_id, team_member['company_id'][0], team_member['crm_team_id'][0], self.id)
